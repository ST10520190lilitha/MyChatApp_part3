...
public class Message {
    ...
    /**
     * Constructor to initialise a newly created Message object.
     */
    ...
    /**
     * Generates a random 10-digit numeric message ID using string manipulation.
     * The first digit is never zero so the result is always 10 digits long.
     *
     * @return a 10-character numeric String to be used as a message ID
     */
    public static String generateMessageID() {
        Random random = new Random();
        // Build a 10-digit ID: first digit 1-9 to ensure correct length
        StringBuilder id = new StringBuilder();
        id.append(random.nextInt(9) + 1); // First digit: 1-9
        for (int i = 1; i < 10; i++) {
            id.append(random.nextInt(10)); // Remaining digits: 0-9
        }
        return id.toString();
    }
     ...
     * Punctuation is stripped from first and last words before concatenating.
     */
    public String createMessageHash() {
        ...
        // Step 4: Strip all non-alphabetic characters (punctuation) from first and last words
        firstWord = firstWord.replaceAll("[^a-zA-Z]", "");
        lastWord = lastWord.replaceAll("[^a-zA-Z]", "");

        // Step 5: Combine fragments separated by colons and convert to uppercase
